...
export const metadata: Metadata = {
  title: `${SITE.streamsLabel} — ${SITE.name}`,
  description: SITE.description,
  alternates: { canonical: `/${SITE.streamsSlug}` },
};
  ...
  return (
    <div className="flex min-h-screen flex-col bg-white text-[#1f2a24] font-body">
      ...
      <main className="mx-auto w-full max-w-6xl flex-1 px-6 py-10">
        <h1 className="font-display text-3xl font-bold leading-tight sm:text-4xl">{SITE.streamsLabel}</h1>
        ...
        {streams.length === 0 ? (
          ...
          <div className="mt-8 grid gap-4 sm:grid-cols-2 lg:grid-cols-3">
            {streams.map((s) => {
              const started = relativeDay(s.uploadDate, now);
              return (
                <VideoCard
                  key={s.slug}
                  video={s}
